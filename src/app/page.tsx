...
export default function Home() {
  ...
  const fetchRanksData = React.useCallback(
    async (_user: Tables<'users'>) => {
      ...
      const currentRank =
        ranks.find((rank) => rank.id === _user.rank_id) || ranks[0];
      setStateRanks(ranks);
  ...
  const fetchUserInteraction = React.useCallback(
    async (user: Tables<'users'>) => {
      const [responseLeaders, responseReferrers, responseQuests] =
        await Promise.all([
      ...
      if (!responseLeaders || !responseReferrers || !responseQuests)
  ...
  const addUserData = React.useCallback(
    async (miniAppData: InitData, tgUser: User) => {
      const reward = tgUser.isPremium
        ? REWARD_PREMIUM_REFERRER
        : REWARD_COMMON_REFERRER;
      const userData: {
        id_tg: number;
      ...
      const responseUserQuests = await addJoinUsersQuests(
        supabase,
        sessionUser.id,
      );
