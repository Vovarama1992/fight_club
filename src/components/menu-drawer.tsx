  ...
  return (
    <Drawer open={isOpen} onClose={() => setIsOpen(false)}>
      <DrawerTrigger asChild disabled={props.isLocked}>
        ...
        >
          ...
          <Image
            ...
            width={128}
            height={128}
            className="lg:w-[600%] lg:h-[600%] w-[90%] h-[90%] object-contain"
            draggable={false}
            priority
          />
          {props.title}
      ...
      <DrawerContent>
            ...
            <Table>
              ...
              <TableBody>
                {props.leaders.map((leader, idx) => (
                  <TableRow key={idx}>
                    <TableCell className="max-w-full text-center truncate text-normal-stroke">
                      {leader.username || 'unidentified'}
                    </TableCell>
                    <TableCell className="max-w-full text-center text-normal-stroke">
        ...
        {props.section === 'friends' && props.friends && (
          <div className="w-full h-full max-h-full flex flex-col mt-4 overflow-auto">
            <div className="w-full flex flex-col gap-2">
              <ReferralRewardCard
                avatarURL={'/icons/referral.png'}
                title={'Invite your friend!'}
                amount={5000}
              />
              <ReferralRewardCard
                avatarURL={'/icons/referral-premium.png'}
            ...
            <div className="w-full flex flex-col gap-2 items-center justify-center mt-2">
              <p className="text-base font-medium text-foreground">
                Referral Multiplier: <span className="text-gold">1.0</span>X
              </p>
              <Button
                variant={'default'}
            ...
            <div className="w-full flex flex-col items-center justify-center mt-4">
              <div className="w-full inline-flex items-center justify-between">
                <p className="text-base text-title-stroke font-medium text-foreground">
                  List of your friends: {props.friends?.length || 0}
                </p>
                <Button
                  variant={'ghost'}
                  size={'icon'}
              ...
              <div className="w-full h-24 bg-backdrop rounded-2xl inline-flex items-center justify-center mt-0.5">
                {props.friends.length > 0 ? (
                  <Table>
                    ...
                    <TableBody>
                      {props.friends.map((friend, idx) => (
                        <TableRow key={idx}>
                          <TableCell className="max-w-full text-center truncate text-normal-stroke">
                            {friend.username || 'unidentified'}
                          </TableCell>
                          <TableCell className="max-w-full text-center text-normal-stroke">
                            {friend.balance.toLocaleString('en')}
                          </TableCell>
                        </TableRow>
                      ))}
                    </TableBody>
                  ...
                  <div>
                    <h4 className="text-base text-white/70 font-medium">
            ...
            <div className="w-full inline-flex gap-2 items-center mt-4">
              <TelegramShareButton
                url={`https://t.me/beeverse_main_dev_bot/start?startapp=frP-${props.user.id_tg}`}
                className="w-full"
              >
                <Button
                  variant={'default'}
                ...
                <Button
                  ...
                  onClick={() => {
                    navigator.clipboard.writeText(
                      `https://t.me/beeverse_main_dev_bot/start?startapp=frP-${props.user.id_tg}`,
                    );
                    tPopup &&
                      tPopup.open({
                        title: 'Done!',
                        ...
                        buttons: [
                          { id: 'btn-ok', type: 'default', text: 'OK' },
                        ],
                      });
                  }}
                >
        ...
        {props.section === 'bonus' && (
          <div className="w-full h-[422px] max-h-full flex flex-col mt-4 overflow-auto"></div>
        )}
