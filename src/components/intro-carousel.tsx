...
export function IntroCarouselDApi(props: { items: IntroItem[]; href: string }) {
            ...
            >
              <div className="min-h-32 flex flex-col gap-2 items-center justify-start px-4">
                <div className="h-16 inline-flex items-start justify-center">
                  <h1 className="text-xl text-foreground font-semibold text-center">
                    {item.title}
                  </h1>
                </div>
                {item.subtitle && (
                  <div className="h-12 inline-flex items-start justify-center">
                    <p className="text-sm text-hint font-semibold text-center">
                      {item.subtitle}
                    </p>
                  </div>
                )}
...
function SlideIcon(props: { isActive: boolean }) {
  return (
    <svg
      ...
      width={20}
      height={20}
      className={`${props.isActive ? 'text-primary fill-primary' : 'text-transparent fill-white/30'} transition-all`}
    >
      <circle
        cx="12"
        cy="12"
